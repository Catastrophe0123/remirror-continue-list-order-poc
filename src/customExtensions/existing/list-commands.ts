	...
	return function ({ tr, dispatch, state }) {
		...
		if (
			...
			if (dispatch) {
				...
				const content =
					listItemType.contentMatch.defaultType?.createAndFill() ||
					undefined;

				wrap = wrap.append(
					Fragment.from(
						listItemType.createAndFill(null, content) || undefined
					)
				);
		...
		if (
			...
			if (dispatch) {
				...
				if (newListItem) {
					...
					tr.setSelection(
						TextSelection.create(tr.doc, newListItemStartPos + 1)
					);
				}
...
type TypeAfter =
	| { type: NodeType; attrs: ProsemirrorAttributes }
	| null
	| undefined;
...
function getAllListItemNames(allExtensions: AnyExtension[]): string[] {
	return allExtensions
		.filter((extension) =>
			extension.tags.includes(ExtensionTag.ListItemNode)
		)
...
function getOrderedListItemTypes(
	...
	for (let depth = sharedDepth; depth >= 0; depth--) {
		...
		if (
			listItemNames.includes(type.name) &&
			!listItemTypes.has(type.name)
		) {
 ...
 */
export function sharedSinkListItem(
	allExtensions: AnyExtension[]
): CommandFunction {
 ...
 */
export function sharedLiftListItem(
	allExtensions: AnyExtension[]
): CommandFunction {
	const listItemNames = getAllListItemNames(allExtensions);
